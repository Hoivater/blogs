...
@section('title') Создать статью @endsection
...
<!-- Новая статья -->
        ...
        <div class="body_menu">
			...
			@endif
            <form name = "add" method = 'post' action = "{{ route('addArticleRN') }}" enctype="multipart/form-data">
        	{{csrf_field()}}
              <div class="mb-3">
                <label for="nameArticle" class="form-label">Название статьи</label>
                <input type="text" class="form-control" id="nameArticle" name = "name" aria-describedby="nameHelp" value = "{{ old('name') }}">
              ...
              <div class="mb-3">
                ...
                <textarea class="form-control" id="descriptionArticle" aria-describedby="descriptionHelp" name = "description">{{ old('description') }}</textarea>
                <div id="descriptionHelp" class="form-text">Необходимо для meta:description, около 150 символов</div>
              </div>
              <div class="mb-3">
                <label for="tags" class="form-label" >Теги</label>
                <input type="text" class="form-control" id="tags" name = "tags" aria-describedby="nameHelp"value = "{{ old('tags') }}" >
                <div id="tags" class="form-text">Вводить через запятую</div>
              </div>
              <div class="mb-3">
              ...
              <div class="mb-3">
                <label for="descriptionArticle" class="form-label">Содержание страницы</label>
		                <div ng-app="textAngularTest" ng-controller="wysiwygeditor" class="container app">
						  ...
						  <div text-angular="text-angular" name="htmlcontent" ng-model="htmlcontent" ta-disabled='disabled'>{{ old('htmlcontent') }}</div>
						  <!--
                ...
                <div class="form-text">Необходимо для meta:description, около 150 символов</div>
              </div>
